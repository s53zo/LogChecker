...
function qslPrintingCard(): string {
  ...
  const status = blocked ? `${blocked} need attention` : `${readiness.ready} ready`;
  return `<section class="card online-tools-card" style="margin-top:1rem"><div class="card-head"><div><p class="eyebrow">Continue online</p><h3>Print or analyze this log</h3></div><span class="format-pill">Optional tools</span></div><div class="card-body"><div class="online-tool-grid"><div class="online-tool"><div><h4>QSL labels and cards</h4><p>Preload this log into ADIF to QSL and choose a print layout.</p></div><div class="online-tool-action"><span class="support-chip">${escapeHtml(status)}</span><button class="btn primary" data-action="open-qsl-printing" ${blocked || !readiness.total ? "disabled" : ""}>Open with this log</button></div></div><div class="online-tool"><div><h4>SH6 log analyzer</h4><p>Explore charts, maps, rates, countries, bands, and operating patterns.</p></div><div class="online-tool-action"><a class="btn dark" href="https://s53m.com/SH6" target="_blank" rel="noopener noreferrer" data-analytics="sh6_recommendation">Open SH6</a></div></div></div>${blocked ? `<div class="status-banner warning compact-status"><span>!</span><div><strong>Complete required QSL fields first.</strong> Missing CALL, QSO_DATE, time, band/frequency, or MODE in records ${escapeHtml(readiness.blocked.slice(0, 10).map((item) => item.recordNumber).join(", "))}${blocked > 10 ? "…" : ""}.</div></div>` : ""}${readiness.alreadySent ? `<p class="help-text">${readiness.alreadySent} record${readiness.alreadySent === 1 ? " is" : "s are"} already marked as sent; review filters before printing again.</p>` : ""}${prepared.warnings.length ? `<details class="export-notes"><summary>${prepared.warnings.length} conversion note${prepared.warnings.length === 1 ? "" : "s"}</summary><ul>${prepared.warnings.map((warning) => `<li>${escapeHtml(warning)}</li>`).join("")}</ul></details>` : ""}<p class="help-text">The QSL handoff goes only to the newly opened tab; log content is never placed in a URL or analytics event.</p></div></section>`;
}
